...
The `test_<model>` aliases send KServe HTTP/REST inference requests with `curl`, print the input tensor shape, and print the response. `test_cpu_models` runs all eight CPU-capable examples. A model must be included in `loadModels` and ready before its test can succeed. DenseNet and Inception also require their downloaded model files. The tests use `TRITON_HTTP_URL`, defaulting to `http://localhost:8000`. If Lima does not expose that address, run `forward_triton` in another terminal to execute `kubectl port-forward` for the Triton service.
